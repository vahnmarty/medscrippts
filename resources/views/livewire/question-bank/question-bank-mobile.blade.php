<div class="bg-gray-100">
        ...
        <div>
            <div>
                <div class="flex">
                    ...
                    <div class="ml-2">
                        <h3 class="text-lg text-darkgreen">Create Qbank</h3>
                    </div>
                </div>
    ...
    <header class="flex items-center justify-between px-6 py-6 lg:px-16 lg:py-6">
        <h1 class="text-2xl font-bold leading-7 lg:text-4xl text-darkgreen sm:leading-9">Question Banks</h1>
        <div>
            <button type="button" x-data x-on:click="$dispatch('openmodal-create')" class="btn-primary">Create</button>
        </div>
    </header>

    <div class="px-6 space-y-3">
        @foreach($qbanks as $qbank)
        <a href="{{ route('qbank.play', $qbank->id) }}" wire:key="card-{{ $qbank->id }}" class="relative block p-5 pr-8 bg-white border border-gray-300 rounded-md">
            <h4 class="text-xl font-bold text-orange-600">{{ $qbank->getCategoriesName() }}</h4>
            ...
            <div class="flex gap-2 mt-2 text-gray-700">
                ...
                <span>{{ $qbank->items_count }} cards</span>
                  
            </div>

            <div class="absolute top-0 bottom-0 right-0 pr-3">
